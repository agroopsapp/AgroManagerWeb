...
interface BreakModalProps {
  ...
  workPartModalMode: "create" | "edit";
  workPartJustSaved: boolean;
  workPartSignatureDialogOpen: boolean;
  workPartSignatureTemp: string | null;
  workPartPdfLoading: boolean;
  ...
  onSetWorkPartModalMode: (mode: "create" | "edit") => void;
  onSetWorkPartSignatureDialogOpen: (v: boolean) => void;
  onSetWorkPartSignatureTemp: (v: string | null) => void;
  onGenerateWorkPartPdf: () => Promise<void>;
}
...
export function BreakModal({
  ...
  onSetWorkPartModalMode,
  onSetWorkPartSignatureDialogOpen,
  onSetWorkPartSignatureTemp,
  onGenerateWorkPartPdf,
}: BreakModalProps) {
  const signaturePreviewSrc =
    workPartSignatureTemp &&
    !workPartSignatureTemp.startsWith("data:") &&
    !workPartSignatureTemp.startsWith("http://") &&
    !workPartSignatureTemp.startsWith("https://")
  ...
  return (
    <div className={`fixed inset-0 z-40 ${MODAL_BACKDROP_SCROLL}`}>
              ...
              <>
                ...
                <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">
                  {workPartModalMode === "edit"
                    ? "Edita el parte existente para este fichaje. Puedes ajustar empresa, servicio y área en las tareas."
                    : "Crea el parte para este fichaje. Elige empresa, servicio y área en cada tarea."}
                </p>
                <div className="mt-3 space-y-1.5 rounded-xl bg-slate-50 p-3 text-sm text-slate-700 dark:bg-slate-700/60 dark:text-slate-100">
                ...
                {signaturePreviewSrc && (
                  <div className="mt-3 rounded-xl border border-slate-200 bg-slate-50/80 p-3 dark:border-slate-600 dark:bg-slate-900/40">
                ...
                <div className="mt-4 flex flex-wrap justify-end gap-2">
                  {workPartModalMode === "edit" && (
                    <div className="mr-auto flex flex-wrap gap-2">
                      <button
                        type="button"
                        onClick={() => onSetWorkPartSignatureDialogOpen(true)}
      ...
      <SignaturePadDialog
        open={workPartSignatureDialogOpen}
        title="Firma del parte"
        initialDataUrl={workPartSignatureTemp}
        onClose={() => onSetWorkPartSignatureDialogOpen(false)}
        onSave={(pngDataUrl) => {
          onSetWorkPartSignatureTemp(pngDataUrl);
          onSetWorkPartSignatureDialogOpen(false);
        }}
      />
    </div>
  );

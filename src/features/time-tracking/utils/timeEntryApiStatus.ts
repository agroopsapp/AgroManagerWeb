...
/**
 * Interpreta el campo `status` del servidor. Cualquier cadena fuera de la lista blanca → `"unknown"`.
 */
export function parseTimeEntryApiStatus(
  ...
  for (const allowed of Array.from(ALLOWED)) {
    if (allowed.toLowerCase() === lower) return allowed as TimeEntryApiStatus;
  }
  return "unknown";
}
...
export function formatTimeEntryStatusForExport(
  ...
  if (status === "FestivoEmpresa") return "Festivo empresa";
  return status;
}
...
export function timeEntryApiStatusBadgeClass(
  ...
  switch (status) {
    ...
    case "FestivoEmpresa":
      return "bg-amber-100 text-amber-950 dark:bg-amber-950/45 dark:text-amber-50";
    case "unknown":
    default:

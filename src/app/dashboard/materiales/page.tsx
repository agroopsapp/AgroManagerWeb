...
import { USER_ROLE, type Material } from "@/types";

type SortKey = "name" | "code" | "unit";
...
export default function MaterialesPage() {
  ...
  const listCompanyId = useMemo(() => {
    const t = tenantCompanyId?.trim();
    if (t) return t;
    const u = user?.companyId?.trim();
    return u || undefined;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setError(null);
      try {
        const list = await materialsApi.getAll({
          signal: ac.signal,
  ...
  return (
    <div className="space-y-4">
        ...
        <div>
          ...
          {isReady && isWorker ? (
            <p className="mt-2 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-600 dark:border-slate-600 dark:bg-slate-900/50 dark:text-slate-300">
              Como trabajador/a, esta pantalla es <strong className="font-semibold">solo consulta</strong>: no puedes
              crear, editar ni eliminar materiales.
            </p>
          ) : null}
        </div>
        {!isWorker ? (
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-3 shadow-sm dark:border-slate-600 dark:bg-slate-800">
        <input
          type="search"
          placeholder="Buscar por nombre, código, unidad o descripción…"
          value={filterSearch}
              ...
              <tr>
                ...
                <th className="hidden px-4 py-3 md:table-cell">
                  {!isWorker ? (
                    ...
                    <span className="font-semibold text-slate-800 dark:text-slate-200">Código</span>
                  )}
                </th>
                <th className="hidden px-4 py-3 sm:table-cell">
                  {!isWorker ? (
                    ...
                    <span className="font-semibold text-slate-800 dark:text-slate-200">Ud.</span>
                  )}
                </th>
                <th className="hidden px-4 py-3 lg:table-cell">
                  <span className="font-semibold text-slate-800 dark:text-slate-200">Descripción</span>
                </th>
                {!isWorker ? (
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-600">
              {filteredSorted.map((row, lineIndex) => (
                <tr key={row.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-700/50">
                  ...
                  <td className="hidden max-w-xs truncate px-4 py-3 text-slate-600 dark:text-slate-400 lg:table-cell">
                    {row.description || "—"}
                  </td>
                  {!isWorker ? (
      ...
      {modalOpen && !isWorker && (
        ...
        >
          <div className={modalScrollablePanel("md")} onClick={(e) => e.stopPropagation()}>
            <h2
              id="mat-form-title"
            ...
            <form onSubmit={handleSave} className="mt-4 space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  required
                  disabled={saving}
                  placeholder="Ej. Abono nitrogenado"
